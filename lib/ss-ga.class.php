...
class ssga {
	...
	private function create_cookie() {
		...
		$rand_id = $this->domainHash($this->data['utmhn']);
		$random = rand( 1000000000, 2147483647 );
		$var = '-';
		$curTime = time();
		// $time = [curTime, curTime, curTime];
		$cookie = '';
		$cookie .= '__utma=' . $rand_id . '.' . $random . '.' . $curTime . '.' . $curTime . '.' . $curTime . '.2;+';
		...
		// 	$cookie .= $_COOKIE['__utma'];
		// 	$rand_id = explode('.', $_COOKIE['__utma'])[0];
		// 	$random = explode('.', $_COOKIE['__utma'])[1];
		// 	$time[0] = intval(explode('.', $_COOKIE['__utma'])[2]);
		// 	$time[1] = intval(explode('.', $_COOKIE['__utma'])[4]);
		// } else {
		// 	$tmpVal = ($rand_id . '.' . $random . '.' . $time[0] . '.' . $time[1] . '.' . $time[2] . '.2');
		// 	$cookie .= $tmpVal;
		// 	setcookie('__utma', $tmpVal, time()+60*60*24*365*2);
		// }
		// $cookie .= ';+';
		// $cookie .= '__utmb=';
		// if(isset($_COOKIE['__utmb'])) {
		// 	$cookie .= $_COOKIE['__utmb'];
		// } else {
		// 	$tmpVal = $rand_id;
		// 	$cookie .= $tmpVal;
		// 	setcookie('__utmb', $tmpVal, time()+60*30);
		// }
		// $cookie .= ';+';
		// $cookie .= '__utmc=';
		// if(isset($_COOKIE['__utmc'])) {
		// 	$cookie .= $_COOKIE['__utmc'];
		// } else {
		// 	$tmpVal = $rand_id;
		// 	$cookie .= $tmpVal;
		// 	setcookie('__utmc', $tmpVal);
		// }
		// $cookie .= ';+';
		// $cookie .= '__utmz=';
		...
		// } else {
		// 	$tmpVal = ($rand_id . '.' . $time . '.2.2.utmccn=(direct)|utmcsr=(direct)|utmcmd=(none)');
		// 	$cookie .= $tmpVal;
		// 	setcookie('__utmz', $tmpVal, time()+60*60*24*365/2);
		// }
		// $cookie .= ';+';
		// $cookie .= '__utmv=';
		// if(isset($_COOKIE['__utmv'])) {
		// 	$cookie .= $_COOKIE['__utmv'];
		// } else {
		// 	$tmpVal = ($rand_id . '.' . $var);
		// 	$cookie .= $tmpVal;
		// 	setcookie('__utmv', $tmpVal, time()+60*60*24*365*2);
		// }
		// $cookie .= ';';
		return $cookie;
	}
	...
	public function set_viewport_resolution( $var = null ) {
		return $this->data['utmvp'] = $var;
	}
